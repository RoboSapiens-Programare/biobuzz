...
public class ForesightTuner extends Procedure {
    ...
    public ForesightTuner(
            Function<HardwareMap, Localizer> localizerFunction, Function<HardwareMap, Drivetrain> drivetrainFunction) {
    ...
    public void run() throws InterruptedException {
        Inputs distanceInput = inputs(
                "Distance", "The distance to drive in inches for the Max Achievable Forward and Strafe Identifiers");
        ...
        Inputs.Field<Double> distanceBraking =
                distanceBrakingInput.d("Distance").withDefault(36.0);
        awaitInputs(distanceBrakingInput);
        double safeDistanceBraking = Math.max(distanceBraking.get(), 15.0);

        List<Double> forwardBraking = runOpMode(new ForwardBraking(
                localizerFunction, drivetrainFunction, headingLinear, headingQuadratic, heading, safeDistanceBraking));
        List<Double> strafeBraking = runOpMode(new StrafeBraking(
                localizerFunction, drivetrainFunction, headingLinear, headingQuadratic, heading, safeDistanceBraking));
        ...
        code(
                ...
                "public static ForesightConfig foresightConfig = new ForesightConfig(\n" + "            c -> {\n"
                        ...
                        + headingLinear + ", " + headingQuadratic + "));\n" + "\n"
                        + "                c.linearBrakeCoefficients.set(Matrix.diag("
                        + forwardLinear + ", " + strafeLinear + "));\n"
                        + "                c.quadraticBrakeCoefficients.set(Matrix.diag("
                        + forwardQuadratic + ", " + strafeQuadratic + "));\n" + "\n"
...
class ForwardVelocity extends TuningOpMode<Double> {
    ...
    public ForwardVelocity(
        ...
        super(
                ...
                "A tuner for finding the maximum achievable forward velocity. This will drive forward for " + distance
...
class StrafeVelocity extends TuningOpMode<Double> {
    ...
    public StrafeVelocity(
        ...
        super(
                "Max Strafe Velocity",
                "A tuner for finding the maximum achievable strafe velocity. This will drive left for " + distance
                        + " inches and then likely drift past that position.",
                false);
        this.localizerFunction = localizerFunction;
        this.drivetrainFunction = drivetrainFunction;
